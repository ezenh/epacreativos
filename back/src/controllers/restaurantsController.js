...
const createRestaurantsClient = async (req, res) => {
    const { _id, event_type, theme, name, lastname, phone, date_start, date_end, __v } = req.body;
        
    try {
        const newClient = await clientsService.createRestaurantsClient({_id, event_type, theme, name, lastname, phone, date_start, date_end, __v});
                
        try {
            // Enviar notificación por WhatsApp
            const message = await clientRequestMessage.messages.create({
                body: `Llegó una nueva solicitud en EPA RESTAURANTES del cliente ${name}, TELÉFONO: ${phone}. \n ID: ${_id}`,
...
module.exports = {
    createRestaurantsClient: catchAsync(createRestaurantsClient),
};
