import { models, events, events_icons, examples } from "./invitationsArrays.js"
    ...
    switch (container.id) {
        case 'models_cards_container':
            source.forEach(model => {
                container.innerHTML += 
                `<article id= '${model.title.split(" ").join("-")}' class ='cards' style= 'background-image: url("./assets/vid/${model.background}")'>
                    <div class= 'model_arrow_container'>
                        <img class= 'model_arrow' src= './assets/ico/up.png'>
                    </div>
                    ...
                    <img class= 'model_logo' src='./assets/logos/${model.logo}'>                    
                </article>
                `
            })
            break;
...
export function checkSliderItemPos(container, dotsContainer) {
    ...
    // console.log(containerArray[0].getBoundingClientRect().left)

    // console.log( "borde izq contenedor = " + container.getBoundingClientRect().left)
    // console.log( "borde der contenedor = " + container.getBoundingClientRect().right)


    // console.log( "borde izq item = " + container.children[1].getBoundingClientRect().left)
    // console.log( "borde der item = " + container.children[1].getBoundingClientRect().right)


    containerArray.forEach(item => {
        if(item.getBoundingClientRect().left >= container.getBoundingClientRect().left && item.getBoundingClientRect().right <= container.getBoundingClientRect().right) {
            centeredItem = item
...
export function animateSliderItems(container, centeredItemIndex) {
    ...
    containerArray.forEach(item => {
        ...
        if(containerArray.indexOf(item) == centeredItemIndex) {
            ...
            switch (container.id) {
                case 'models_cards_container':
                    // card.style.boxShadow = '0px 0px 50px 50px rgba(255, 255, 255, 0.673)'
